...
public:
	...
	/// <summary>초기화 함수입니다. </summary>
	/// <param name="type">스크롤 타입입니다. scroll::SCRType::SCR_H 와 같은 방식으로 작성됩니다.</param>
	/// <param name="ScrLength">스크롤할 원본 데이터 길이입니다. </param>
	/// <param name="ScrViewLength">화면에 보이는 데이터 길이입니다. </param>
	/// <param name="ScrollLength">스크롤바의 길이입니다. </param>
	/// <param name="ThumbLength">
	/// <para>( = 0.f) : 스크롤 바의 이동시킬 수 있는 막대 길이(1/2)입니다. 0을 입력하면 자동으로 계산합니다. </para>
	/// <para> ※ ThumbLength의 길이를 자동으로 조절할 수 있는 조건 : ScrollLength > ThumbLength * 2 </para>
	/// </param>
	/// <param name="thickness">스크롤의 굵기입니다. </param>
...
public:
	...
	/// <param name="ScrViewLength">화면에 보이는 데이터 길이입니다. </param>
	/// <param name="ScrollLength">스크롤바의 길이입니다. </param>
	/// <param name="WorldPosition">스크롤이 그려질 영역의 전역 시작 좌표입니다.</param>
	/// <param name="Opacity">스크롤의 투명도입니다. 0부터 255까지의 정수입니다. </param>
	/// <param name="ThumbLength">
	/// <para>( = 0.f) : 스크롤 바의 이동시킬 수 있는 막대 길이(1/2)입니다. 0을 입력하면 자동으로 계산합니다. </para>
	/// <para> ※ ThumbLength의 길이를 자동으로 조절할 수 있는 조건 : ScrollLength > ThumbLength * 2 </para>
	/// </param>
...
/*
* basic_scroll을 상속받은 Direct2D에서 사용이 가능한 scroll입니다.
* OnProcessingMouseMessage를 이용하여 스크롤을 이용할 때 필요한 마우스입력을 간단히 사용할 수 있도록 했습니다.
* 스크롤을 사용할 시점에 initialize를 한 뒤 그 이후 OnProcessingMouseMessage를 호출해주고, WM_PAINT에서 Draw 함수를 호출해 주는 것으로 사용할 수 있습니다.
* 스크롤을 이용하여 변경할 데이터는 GetNowSrcPosition을 이용해 값을 읽을 수 있습니다.
* 아래 클래스를 참고하여 다른 그래픽라이브러리에서 동작하는 스크롤 클래스를 만들 수 있습니다.
*/
class D2D_SRCOLL : public basic_scroll {

	bool PtInRect(const D2D_RECT_F* rc, const D2D_POINT_2F pt)
	{
...
protected:
	...
	float				m_fOpacity;
	//	스크롤 영역입니다. 이 영역대로 그립니다.
	D2D_RECT_F			m_rcScroll;
	//	Thumb의 영역입니다. 이 영역대로 그립니다.
	D2D_RECT_F			m_rcThumb;
	//	현재 마우스가 눌렸는지를 반환합니다.
	bool				m_bOnClick;
	//	스크롤이 그려지는 전역 좌표입니다. 이 좌표를 원점으로 하여 스크롤이 그려집니다.
	D2D_POINT_2F		m_ptWorldPosition;
...
public:
	...
	/// </param>
	/// <param name="thickness">스크롤의 굵기입니다. </param>
	virtual void initialize(SCRType type, float ScrLength, float ScrViewLength, float ScrollLength, D2D_POINT_2F WorldPosition,
		float Opacity = 1.0f, float ThumbLength = 0.f, float thickness = 5.0f)
	{
		basic_scroll::initialize(type, ScrLength, ScrViewLength, ScrollLength, ThumbLength, thickness);
		m_ptWorldPosition = WorldPosition;
		m_fOpacity = Opacity;
		...
		if (m_scType == SCRType::SCR_H)
			...
			m_ptWorldPosition.x,
			m_ptWorldPosition.y,
			m_ptWorldPosition.x + m_flScroll + 2 * m_flThumb,
			m_ptWorldPosition.y + m_flThickness };
		else
			m_rcScroll = {
			m_ptWorldPosition.x,
			m_ptWorldPosition.y,
			m_ptWorldPosition.x + m_flThickness,
			m_ptWorldPosition.y + m_flScroll + 2 * m_flThumb };
	...
	* 마우스가 눌려졌을 때의 상황을 처리합니다.
	*/
	bool TouchCheck(D2D_POINT_2F pt, float ScrollmoveLength = 10.f)
	{
		...
		{
			if (pos > m_flThumb + m_fnowScrollpos)
				AdjustnowScrollPosition(ScrollmoveLength);
			else
				AdjustnowScrollPosition(-ScrollmoveLength);
			RenewalThumbRect();
			m_bOnClick = true;
	...
	*/
	void RenewalThumbRect()
	{
		if (m_scType == SCRType::SCR_H)
			m_rcThumb = {
			m_ptWorldPosition.x + m_fnowScrollpos,
			m_ptWorldPosition.y - 1,
			m_ptWorldPosition.x + 2 * m_flThumb + m_fnowScrollpos,
			m_ptWorldPosition.y + m_flThickness + 1 };
		else
			m_rcThumb = {
			m_ptWorldPosition.x - 1,
			m_ptWorldPosition.y + m_fnowScrollpos,
			m_ptWorldPosition.x + m_flThickness + 1,
			m_ptWorldPosition.y + 2 * m_flThumb + m_fnowScrollpos };
	}
	...
	*/
	virtual void Draw(ID2D1RenderTarget *pd2dRenderTarget, const D2D_POINT_2F& ptWorldPosition = Point2F(0.f, 0.f), ColorF color = ColorF::Black)
	{
		...
		sbr->SetColor(color);
		pd2dRenderTarget->DrawRoundedRectangle(RoundedRect(m_rcScroll, m_flThickness * 0.5f, m_flThickness * 0.5f), sbr);
		pd2dRenderTarget->FillRoundedRectangle(RoundedRect(m_rcThumb, m_flThickness * 0.5f, m_flThickness * 0.5f), sbr);
